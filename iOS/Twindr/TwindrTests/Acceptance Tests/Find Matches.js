...
target.frontMostApp().mainWindow().buttons()["Find Matches"].tap();
UIATarget.localTarget().delay(1);

// Check for a match to press yes to
if(target.frontMostApp().toolbar().buttons()["Yes"].checkIsValid()){
    target.frontMostApp().toolbar().buttons()["Yes"].tap();
    UIATarget.localTarget().delay(1);
}

// Check for a match to press no to
if(target.frontMostApp().toolbar().buttons()["No"].checkIsValid()){
    target.frontMostApp().toolbar().buttons()["No"].tap();
    UIATarget.localTarget().delay(1);
}
