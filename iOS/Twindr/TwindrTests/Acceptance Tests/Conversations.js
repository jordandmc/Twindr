...
target.frontMostApp().mainWindow().buttons()["Conversations"].tap();
UIATarget.localTarget().delay(1);

// If we have a match, send a message
    target.frontMostApp().mainWindow().tableViews()[0].tapWithOptions({tapOffset:{x:0.17, y:0.02}});
    UIATarget.localTarget().delay(1);
if(target.frontMostApp().navigationBar().name() != "Matches"){
    target.frontMostApp().toolbar().textViews()[0].tapWithOptions({tapOffset:{x:0.11, y:0.40}});
    UIATarget.localTarget().delay(1);
    target.frontMostApp().keyboard().typeString("Hello");
    UIATarget.localTarget().delay(1);
    target.frontMostApp().toolbar().buttons()["Send"].tap();
    UIATarget.localTarget().delay(1);
